...
<title>Clear my bin &#8211; Towards A Greener Tomorrow</title>
...
<link rel="canonical" href="https://towardsagreenertomorrow.ml/temp/" />
<link rel='shortlink' href='https://towardsagreenertomorrow.ml/?p=1935' />
<link rel="alternate" type="application/json+oembed" href="https://towardsagreenertomorrow.ml/wp-json/oembed/1.0/embed?url=https%3A%2F%2Ftowardsagreenertomorrow.ml%2Ftemp%2F" />
<link rel="alternate" type="text/xml+oembed" href="https://towardsagreenertomorrow.ml/wp-json/oembed/1.0/embed?url=https%3A%2F%2Ftowardsagreenertomorrow.ml%2Ftemp%2F&#038;format=xml" />
...
<body data-rsssl=1 itemtype='https://schema.org/WebPage' itemscope='itemscope' class="page-template-default page page-id-1935 wp-custom-logo theme-astra woocommerce-no-js ehf-template-astra ehf-stylesheet-astra ast-desktop ast-separate-container ast-two-container ast-no-sidebar astra-2.0.1 ast-header-custom-item-inside ast-single-post ast-inherit-site-logo-transparent ast-normal-title-enabled elementor-default">
...
<article 
	...
	class="post-1935 page type-page status-publish ast-article-single" itemtype="https://schema.org/CreativeWork" itemscope="itemscope" id="post-1935">
...
<div id="floating-panel">
	<input id="address" type="textbox" placeholder="Type your location here">
...
<script>
	...
	function initMap() {
		...
		document.getElementById('submit').addEventListener('click', function() {
			change_location(geocoder, map);
		});
		
		function change_location(geocoder, resultsMap) {
			for (var i = 0; i < markers.length; i++) {
				markers[i].setMap(null);
				
			}
			markers = [];
			document.getElementById('collect').innerHTML = '<tr><th>Title</th><th>Address</th><th>Contact</th></tr>';
			...
			geocoder.geocode({'address': address}, function(results, status) {
				if (status === 'OK') {
					...
					infoWindow.setContent('Your location');
					infoWindow.open(map);
					getNearbyPlaces(results[0].geometry.location);
				} else {
		...
		function createMarkers(places) {
			...
			places.forEach(place => {
				...
				service.getDetails({reference: place.reference}, (placeResult, status) => {
					let phone = "None";
					if (placeResult.formatted_phone_number) phone = placeResult.formatted_phone_number;
					document.getElementById('collect').innerHTML = document.getElementById('collect').innerHTML + 
						'<tr><td><a href=' + placeResult.website + '>' + placeResult.name + '</a></td><td>' + placeResult.formatted_address + '</td>' +
						'<td>' + phone + '</td></tr>';
				});
				...
				//document.getElementById('collect').innerHTML = document.getElementById('collect').innerHTML + '<tr><td>' + marker.title + '</td><td>' + marker.address + '</td></tr>';
		...
		function showDetails(placeResult, marker, status) {
			if (status == google.maps.places.PlacesServiceStatus.OK) {
				...
				if (placeResult.formatted_phone_number) contact = placeResult.formatted_phone_number;
				placeInfowindow.setContent('<div><strong>' + placeResult.name + '<br>' +
					'<li>Address:</li>' + address + '<br>' +
					'<li>Website:</li>' + '<a href=' + website + '>' + website + '</a><br>' +
...
<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCpZEWUsgHIsLOqQrN70a-xvgNgSnHw4JM&libraries=places&callback=initMap">
</script>
